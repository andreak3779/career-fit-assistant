...
`scripts/merge_pluralsight_course.py` (`add` / `add-batch` subcommands, both with `--dry-run`) is the write-side counterpart for `pluralsight-courses.md` (~587 lines / ~15.5K tokens) — it inserts new completed courses/labs in place (chronological ordering, section/heading/Summary-table counts, Labs sub-table creation) so `pluralsight-updater` never has to re-emit the whole file by hand. `scripts/update_learning_history.py` (`apply` subcommand, `--dry-run`) is the equivalent for `data/pluralsight_learning_history.json` — it merges a batch of `{name: summary}` pairs in place instead of requiring `pluralsight-summary-enricher` to rewrite the full ~44K-token file on every backfill checkpoint. Both take `--root` for testability (see `tests/test_merge_pluralsight_course.py`, `tests/test_update_learning_history.py`); tests must point them at a fixture, never the real files.
